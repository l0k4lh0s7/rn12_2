...
const DateTimePicker = ({
  ...
  const [selectedTime, setSelectedTime] = useState(
    isEditModal ? hour && minute ? handleReadableDate(hour, minute) : 'Select time' : 'Select time'
  );
  ...
  const handleConfirmDate = (date) => {
    if (passAll) {
      passHourAndMinutes(date);
    } else {
      passHour(date.getHours());
      passMinutes(date.getMinutes());
    }

    const formattedTime = handleReadableDate(date.getHours(), date.getMinutes());
